...
export default function ProjectManagementPage() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const missingCount = workRequests.filter(wr => (wr as any).customer_missing || wr.customer_name === 'Missing Customer').length
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        ...
        {activeTab !== 'work-requests' && renderPlaceholderTab(tabs.find(t => t.id === activeTab)?.label || 'Module')}
