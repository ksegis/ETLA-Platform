...
export default function GroupReportsPage({ params }: { params: { group: GroupKey } }) {
  const group = (params.group ?? "employee") as GroupKey;
    ...
    <div className="mx-auto max-w-6xl">
      <section className="mb-4">
        <h2 className="text-base font-semibold text-gray-900">
          {group === "all" ? "All Reports" : `${GROUP_LABELS[group as keyof typeof GROUP_LABELS] ?? "Reports"}`}
        </h2>
      ...
      >
        <div className="flex items-center justify-between border-b border-gray-200 p-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-semibold text-gray-900">{selected?.title ?? "Configure Report"}</h3>
          </div>
          <button
