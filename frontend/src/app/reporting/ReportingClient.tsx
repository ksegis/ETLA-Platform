...
export default function ReportingClient() { // Renamed component
  ...
  const fetchDataForTab = useCallback(async (tabId: string, tenantId: string) => {
    setLoading(true);
    setError(null);
    try {
      ...
      } else if (!isMultiTenant && selectedTenant) {
      ...
      switch (tabId) {
        case "employees":
          setEmployeeData(data as EnhancedEmployee[]);
          break;
        case "pay-statements":
          setPayStatementData(data as EnhancedPayStatement[]);
          break;
        case "timecards":
          setTimecardData(data as EnhancedTimecard[]);
          break;
        case "jobs":
          setJobData(data as JobRecord[]);
          break;
        case "tax-records":
          setTaxData(data as TaxRecord[]);
          break;
        case "benefits-deductions":
          setBenefitData(data as BenefitDeduction[]);
          break;
        case "compliance":
          setComplianceData(data as ComplianceRecord[]);
          break;
      }
    } catch (err: any) {
  ...
  }, [isMultiTenant, tenantFilter, selectedTenant]);

  useEffect(() => {
    if (tenantLoading) return;
    const targetTenantId = isMultiTenant ? tenantFilter : selectedTenant?.id;
    if (targetTenantId) {
      fetchDataForTab(activeTab, targetTenantId);
    }
  }, [activeTab, tenantFilter, selectedTenant, isMultiTenant, tenantLoading, fetchDataForTab]);
              ...
              <CardHeader>
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex items-center gap-2">
                    <Button variant="outline" size="icon" onClick={() => fetchDataForTab(activeTab, isMultiTenant ? tenantFilter : selectedTenant?.id || "")} disabled={loading}>
      ...
      {showNewFacsimileModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
          <div className="bg-white p-8 rounded-lg shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto relative">
            ...
            {/* The content of the new facsimile modal will be rendered here */}
            {/* This could be a dynamic component based on newFacsimileType */}
