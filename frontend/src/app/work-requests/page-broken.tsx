  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const requestData = {
        ...
        required_completion_date:
          formData.required_completion_date || undefined,
      };

      if (workRequest) {
        await workRequestService.updateWorkRequest(workRequest.id, requestData);
      } else {
        await workRequestService.createWorkRequest(
          requestData,
          user.id,
          tenantId,
        );
            ...
            <div>
              ...
              <input
                ...
                onChange={(e: any) =>
                  setFormData({
                    ...formData,
                    required_completion_date: e.target.value,
                  })
                }
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
...
export default function WorkRequestsPage() {
  ...
  const loadWorkRequests = async () => {
    ...
    try {
      ...
      if (tenantUser?.role === "client_user" && user) {
        requests = await workRequestService.getWorkRequestsForUser(
          user.id,
          tenantId,
        );
      } else {
      ...
      // Load stats
      const requestStats =
        await workRequestService.getWorkRequestStats(tenantId);
  ...
  useEffect(() => {
    ...
    if (priorityFilter) {
      filtered = filtered.filter(
        (request: any) => request.priority === priorityFilter,
      );
    }
        ...
        <div className="p-6">
          <div className="text-center py-12">
            ...
            <p className="text-gray-600">
              You don't have permission to view work requests.
            </p>
          </div>
        </div>
      </DashboardLayout>
      ...
      <div className="p-6">
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center gap-4">
            <TenantSelector />
            {canView(FEATURES.WORK_REQUESTS) &&
              canManage(PERMISSIONS.WORK_REQUESTS_CREATE) && (
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <span
                            className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${priorityStyle.bg} ${priorityStyle.color}`}
                          >
                            {priorityStyle.label}
                          </span>
                        </div>
                        ...
                        <div className="flex items-center gap-6 text-sm text-gray-500">
                          ...
                          {request.estimated_hours && (
                            <div>Est. {request.estimated_hours}h</div>
                          )}
                          {request.budget && (
                      ...
                      <div className="flex items-center gap-2 ml-4">
                        ...
                        {canManage(PERMISSIONS.WORK_REQUESTS_UPDATE) && (
                          <Button
                            variant="outline"
                            size="sm"
                        ...
                        {canManage(PERMISSIONS.WORK_REQUESTS_DELETE) && (
                          <Button
                            variant="outline"
                            size="sm"
