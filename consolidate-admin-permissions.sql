...
-- 3. (Optional) If you want to grant host_admin access to all tenants, run this:
-- UPDATE tenant_users
-- SET
--   role = 'host_admin',
--   role_level = NULL,
--   is_primary_tenant = (tenant_id = (SELECT id FROM tenants WHERE name = 'Demo Company')),
--   is_active = true,
...
-- 4. Verify the final permissions for Kevin Shelton
SELECT 
    p.email,
    p.full_name,
    tu.role,
